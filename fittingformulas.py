...
# Method 2: deg 4
A4 = design(x1, x2, x3, trips(4))
pred_fr_d4 = np.exp(A4 @ np.linalg.lstsq(A4, ln_fr, rcond=None)[0])
pred_frho_d4 = np.exp(A4 @ np.linalg.lstsq(A4, ln_frho, rcond=None)[0])
...
# Method 4: log-poly 15 terms
lf = np.log(1 + fb/0.1)
le = np.log(eta)
A_lp = np.column_stack([np.ones(N), Xi, Xi**2, lf, le, lf**2, le**2, lf*le,
                         Xi*lf, Xi*le, Xi*lf*le, Xi**2*lf, lf**2*le, lf*le**2, Xi*lf**2])
pred_fr_lp = np.exp(A_lp @ np.linalg.lstsq(A_lp, ln_fr, rcond=None)[0])
pred_frho_lp = np.exp(A_lp @ np.linalg.lstsq(A_lp, ln_frho, rcond=None)[0])

# Sorted errors
def serr(t, p): return np.sort(np.abs((p-t)/t)*100)

err_fr  = [serr(f_r, p) for p in [pred_fr_d5, pred_fr_d4, pred_fr_sp, pred_fr_lp]]
err_frho = [serr(f_rho, p) for p in [pred_frho_d5, pred_frho_d4, pred_frho_sp, pred_frho_lp]]
pct = np.linspace(0, 100, N)

colors = ['#1b9e77', '#d95f02', '#7570b3', '#e7298a']
styles = ['-', '-', '--', ':']
lws = [2.5, 2.0, 2.0, 1.8]
names = ['Chebyshev deg 5  (56 terms)', 'Chebyshev deg 4  (35 terms)',
         'Sparse Chebyshev (30 terms)', 'Log-polynomial    (15 terms)']

fig, axes = plt.subplots(1, 2, figsize=(13.5, 5.5))

# vertical offsets for the endpoint labels so they don't overlap
# [deg5, deg4, sparse, logpoly]
fr_offsets  = [(8, -2), (-35, 6), (-35, -12), (-35, 6)]
frho_offsets = [(8, -2), (-35, 6), (-35, -12), (-35, 6)]
...
for panel, (ax, errs_list, title, offsets) in enumerate(zip(
    ...
    if panel == 0:
        ...
        ax.set_ylim(0, min(err_fr[3][-1]*1.2, 8))
    else:
        for h in [5, 10]:
            ax.axhline(h, color='gray', ls='--', lw=0.7, alpha=0.5)
            ax.text(2, h + 0.3, f'{h}%', fontsize=8, color='gray', alpha=0.6)
        ax.set_ylim(0, min(err_frho[3][-1]*1.2, 14))

    ax.axvline(95, color='gray', ls=':', lw=0.7, alpha=0.4)
    ax.text(95.5, ax.get_ylim()[0] + 0.1, 'p95', fontsize=8, color='gray')
    ax.set_xlim(0, 100)
